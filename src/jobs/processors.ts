...
import {
  ...
  ladderCrawl,
  ladderWalk,
} from './ladder-crawl.js';
import { walkIsComplete, walkMatchIds } from './match-walk.js';
import { backfillNames } from './player-names.js';

/**
 * The per-player jobs — polling, archiving and backfill — and `dispatch`, which
 * routes every job name in the service to its handler.
 *
 * It used to hold all of them. #122 moved the three that had grown their own
 * vocabulary into files of their own: the crawl state machine
 * (`ladder-crawl.ts`), the archive recomputes (`analytics.ts`) and the name
 * backfill (`player-names.ts`). `dispatch` stays here because routing a job
 * name to a handler is a smaller job than defining one, and one switch that
 * names every job is worth more than four partial ones.
 *
...
export async function backfillPlayer(
  ...
  const { depth, ranOut } = await walkMatchIds(
    region,
    puuid,
    { limit, fetch: BULK, ...(queueId !== undefined ? { queueId } : {}) },
    async (ids, start) => {
